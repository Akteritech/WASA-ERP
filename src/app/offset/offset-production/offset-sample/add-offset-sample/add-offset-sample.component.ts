...
declare var $: any;
@Component({
  selector: 'app-add-offset-sample',
  templateUrl: './add-offset-sample.component.html',
  styleUrls: ['./add-offset-sample.component.css']
})
export class AddOffsetSampleComponent implements OnInit {
  collapse = false;
  collapse1 = false;
  addsample: any;
  companyid: any;
  ...
  constructor(public api: ApiService, private route: ActivatedRoute, private router: Router, private _location: Location , public modalService: SuiModalService ,
    ...
    // this.addsample.length = '';
    // this.addsample.width = '';
    // this.addsample.totalnoofcolor = '';
    // this.addsample.papergsm = '';
    // this.addsample.noofups = '';
    this.addsample.productcategoryid = 5;
    this.part.partnoid = 1216;
    // this.addsample.id = 11;
...
if (this.color.samplecolorname === '') {
  ...
  getproductcategories() {
    this.api.getdata('productcategories?filter[where][productcategoryid]=5').subscribe((res: any) => {
        ...
        this.api.patchdata('SampleGeneralSpecs/addSample', this.addsample).subscribe((res: any) => {
          ...
          setTimeout(() => {
            this.api.getdata('SampleGeneralSpecs?filter[order]=sampleid DESC&filter[where]' +
                '[productcategoryid]=5&filter[limit]=1').subscribe((resp: any) => {
              this.samples = resp;
              console.log(resp);
              this.sampleid = resp[0].sampleid;
              this.part.sampleid = resp[0].sampleid;
              this.part.length = resp[0].length;
              this.part.width = resp[0].width;
              this.part.createdby = sessionStorage.getItem('userid');
              this.api.patchdata('SampleWisePartsLengths', this.part).subscribe(res2 => {
                this.response = res2;
              }, err => {
...
//  }
    this.api.patchdata('SampleGeneralSpecs/updateSample', this.addsample).subscribe((res: any) => {
      ...
      this.router.navigateByUrl('offset/samples');
    }, err => {
      this.api.showFailureToast('Error', err.message);
      console.log(err);
    });
    ...
    setTimeout(() => {
      if (!this.editId) {
        this.api.getdata('SampleGeneralSpecs?filter[order]=sampleid ' +
            'DESC&filter[where][productcategoryid]=5&filter[limit]=1').subscribe((res: any) => {
          this.samples = res;
