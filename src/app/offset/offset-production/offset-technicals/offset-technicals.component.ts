...
@Component({
  selector: 'app-offset-technicals',
  templateUrl: './offset-technicals.component.html',
  styleUrls: ['./offset-technicals.component.css']
})
export class OffsetTechnicalsComponent implements OnInit {
  ...
  // alert(id) {
  //   this.sample.customersamplename  = id ;
  // }
  //
  // optionsSearch = (query) => {
  //   this.searchBox.dropdownService.setOpenState(true);
  //   return this.api.getdata('SampleGeneralSpecs?filter={ "where":{"customersamplename":{"like":"%25' + query + '%25"}}}').toPromise();
  // }
  ngOnInit() {
  ...
  getFilterUrl() {
    ...
    filter['order'] = 'offsettechnicalid DESC';
  ...
  get(search?: boolean) {
    ...
    this.api.getdata('offset-technical-details?page=' + this.meta.currentPage + '&filter=' + JSON.stringify(this.getFilterUrl())).subscribe((res: any) => {
