...
@Component({
  selector: 'app-pfl-sample',
  templateUrl: './pfl-sample.component.html',
  styleUrls: ['./pfl-sample.component.css']
})
export class PflSampleComponent implements OnInit {
  ...
  generateExcel() {
    ...
    console.log(this.selectedSamples);
      const data = this.selectedSamples.map((element: any) => {
        return {
          'SampleName': element['samplename'], 'SampleNo': element['sampleno'], 'Product Category': element.productCategory.productcategoryname,
          'Product Sub Category': element.productSubCategory.productsubcategoryname, 'Creation Date': element['creationdate'], 'Creation By': element['createdby'],
          'Brand Name': element.brand.brandname,'Program Name': element.program.programname,'Length': element['length'],
          'Width': element['width'],'Submission Date': element['submissiondate'], 'Status': element.status.statusdescription,
          'Approval Date': element['approveldate'], 'Comment': element['remark2'],
          'Company': element.company.companyname
        };
      });
      const lengths = [22, 10, 30, 30, 11, 15, 25, 20, 20, 8, 13, 10, 10, 10,18];
      this.api.exportExcel(data, [['Pfl Sample Report'], []], 'Pfl Sample Report',  lengths);
  }
  ...
  viewDetail(id) {
    console.log(id);
    this.modalService
      .open(new ShowDetail('Sample Details', 'pflSample', id))
      .onApprove(() => {
