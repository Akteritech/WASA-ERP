...
declare var $: any;
@Component({
  selector: 'app-add-screen-print-sample',
  templateUrl: './add-screen-print-sample.component.html',
  styleUrls: ['./add-screen-print-sample.component.css']
})
export class AddScreenPrintSampleComponent implements OnInit {
  collapse = false;
  addsample: any;
  companyid: any;
  sampleid = 0;
  ...
  constructor(public api: ApiService, public modalService: SuiModalService, private route: ActivatedRoute, private router: Router, private _location: Location) {
    ...
    this.addsample.productcategoryid = 6;
    // this.color.partnoid = this.color.partnoid;
...
if (this.color.samplecolorname === '') {
  ...
  getSample() {
    setTimeout(() => {
      this.api.getdata('SampleGeneralSpecs?filter[where][customersamplename]=' + this.addsample.samplename
       + '&filter[where][length]=' + this.addsample.length + '&filter[where][width]='
        + this.addsample.width + '&filter[where][productcategoryid]=' + this.addsample.productcategoryid).subscribe((res1: any) => {
        console.log(res1);
        if (res1.length < 1) {
          console.log();
        } else {
  ...
  Updatepatch(form: NgForm) {
    ...
    // delete(this.addsample.artworklocation);
    // delete(this.addsample.filename);
//     if (this.addsample.issubcontact = false) {
//       this.addsample.issubcontact = 0;
//  } else if (this.addsample.issubcontact = true) {
//    this.addsample.issubcontact = 1;
//  }
    this.api.patchdata('SampleGeneralSpecs/updateSample', this.addsample).subscribe((res: any) => {
      ...
      this.router.navigateByUrl('screen-print/samples');
    }, err => {
      this.api.showFailureToast('Error', err.message);
      console.log(err);
    });
...
//   elements.lov.forEach(data => {
//     this.partno = data.listitem;
//     this.no = +this.partno;
//     this.part.partsid = elements.partsid;
//     this.part.partnoid = elements.partnoid;
//     this.part.length = (this.addsample.length * this.no );
...
//   })
// } }
  updatePartNo(): Promise<object> {
    return new Promise<object>((resolve, reject) => {
      this.api.getdata('SampleGeneralSpecs?filter[where][sampleid]=' + this.sampleid).subscribe((res: object) => {
        resolve(res);
        console.log(res);
      });
    });
 ...
 async arrayData() {
for (const {partnoid, sampleid, partsid, lov: { listitem }} of this.SamplePartWiseData) {
// const bio = `${partnoid} is a ${listitem}`;
const lovno = +listitem;
// console.log(bio);
   await this.api.getdata('SampleGeneralSpecs?filter[where][sampleid]=' + this.sampleid).subscribe((res: any) =>{
      this.samplelength = res[0].length;
      this.samplewidth = res[0].width;
    }
);
  this.part.length = (this.samplelength * lovno);
  this.part.partnoid = partnoid;
  this.part.partsid = partsid;
  this.part.sampleid = sampleid;
  this.part.width = this.samplewidth;
 await this.api.patchdata('SampleWisePartsLengths', this.part).subscribe(respo => {
    this.response = respo;
    console.log(respo);
    // this.api.showSuccessToast('Sample Part Wise Length And Width Also Updated Successfully ', this.response.message);
    this.part.partnoid = '';
  ...
  updatepart() {
    console.log('sdf');
    this.SamplePartWiseData.forEach(innerItem => {
      this.part.partnoid = innerItem.partnoid;
      console.log(this.part.partnoid);
      this.api.getdata('lovData?filter[where][id]=' + this.part.partnoid).subscribe((res: any) => {
        this.lovPartNo = res[0].listitem;
        this.lovno = +this.lovPartNo;
        console.log(this.lovno);
        this.api.getdata('SampleGeneralSpecs?filter[where][sampleid]=' + this.sampleid).subscribe((data: any ) => {
          this.MySampleLengthIS = data[0].length;
        });
        setTimeout( () => {
          this.part.length = (this.MySampleLengthIS * this.lovno);
          this.part.partnoid = innerItem.partnoid;
          this.part.partsid = innerItem.partsid;
          this.part.sampleid = innerItem.sampleid;
          this.part.width = this.addsample.width;
          this.api.patchdata('SampleWisePartsLengths', this.part).subscribe(respo => {
            this.response = respo;
            console.log(respo);
            // this.api.showSuccessToast('Sample Part Wise Length And Width Also Updated Successfully ', this.response.message);
            this.part.partnoid = '';
